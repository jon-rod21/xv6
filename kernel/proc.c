 ...
 * total tickets for a for all processes
 */
int total_tickets;
...
fork(void)
{
  ...
  *np->tf = *proc->tf;
  ...
  /* The following code is added by jonathan rodriguez jdr220004
   * the forked child will have the same tickets as the parent
  */
	acquire(&ptable.lock);
...
exit(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
	...
	/* The following code has been added by jonathan rodriguez jdr220004
	 * removes tickets when a process exits
	 */
...
{
  struct proc *p;
	/* The following code is added/modified by jonathan rodriguez jdr220004
